...
setwd("~/nanjing/GDC_TCGA_ Liver_ Cancer")

DF<-read.table("tmp01", head=TRUE)
...
cts_round<-round(cts)
countData<-cts_round
condition <- factor(c(rep("p",289),rep("m",103)), levels = c("p","m"))
...
dds <- dds[keep,]
dim(dds)
dds$condition <- factor(dds$condition, levels = c("p","m"))
dds$condition <- relevel(dds$condition, ref = "m")
dds$condition <- droplevels(dds$condition)
dds <- estimateSizeFactors(dds)
dds_norm<-counts(dds,normalized=TRUE)
head(dds_norm)
write.csv(dds_norm,"p_vs_m_norm.csv")
dds2<-DESeq(dds)
resultsNames(dds2)
res <- results(dds2)
res <- results(dds2, contrast=c("condition","p","m"))
resultsNames(dds2)
write.csv(res,"p_vs_m_deg.csv")


DF<-read.table("tmp02", head=TRUE)
b<-aggregate(DF[, -c(1:2)], by=list(DF$EntrezID, DF$Name), mean)
cts<-b[,-1]
write.table(cts,"cts")
cts<-read.table("cts",head=TRUE,row.names = "Group.2")
...
cts_round<-round(cts)
countData<-cts_round
condition <- factor(c(rep("m",103),rep("p",289)), levels = c("m","p"))
coldata<-data.frame(row.names=colnames(countData),condition)
condition
coldata
dds<-DESeqDataSetFromMatrix(countData=countData, colData=coldata, design=~condition)
